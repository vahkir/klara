...
recognizer.onresult = function (event) {
    ...
    user.innerText = result[0].transcript;
    ...
    if (result.isFinal && !boollang) {
      if(slovo == 'здравствуй' || slovo == 'привет' || slovo == 'дружище'|| slovo == 'привет-привет'|| slovo == 'привет дружище' || slovo == 'дружище привет' || slovo == 'здорово' || slovo == `здорово ${robname}` || slovo == `здравствуй ${robname}` || slovo == `привет ${robname}` || slovo == `${robname} привет` || slovo == `${robname} здравствуй`){
      ...
      }else if(slovo == 'как тебя зовут' || slovo == 'кто ты' || slovo == 'кто ты такой' || slovo == 'ты кто' || slovo == 'как твоё имя' || slovo == 'как там твоё имя' || slovo == 'давай познакомимся' || slovo == 'давайте познакомимся' || slovo.includes('знакомиться будем') || (artcant == 1 && slovo.includes('ничего ещё раз будем познакомиться') || slovo.includes('Ничего ещё раз познакомимся')) || (artcant == 2 && slovo.includes('ну я тебя не помню') || slovo.includes('давай ещё раз будем знакомиться') || slovo.includes('ну я не помню') || slovo.includes('я забыл') || slovo.includes('я не услышал'))){
        if(artcant == 0) {
          xosq(`меня зовут ${robname}, я персональный помощник, буду рада помочь.`);
          artcant++;
        }else if(artcant == 1) {
        ...
        radio1.checked ? xosq('Клара — женское личное имя. Происходит от латинского слова clara, означающего «светлая».'):xosq('Давид — мужское русское личное имя древнееврейского происхождения, в переводе означает «возлюбленный», «любимый», «любимец»');
